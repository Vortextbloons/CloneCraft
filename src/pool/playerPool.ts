...
export class PlayerPool {
  ...
  activePlayers(): Player[] { return world.getAllPlayers().filter(p => { const r = this.players.get(p.id); return (r?.phase === "active" || r?.phase === "applying") && this.eligible(p); }).sort((a, b) => a.id.localeCompare(b.id)); }
