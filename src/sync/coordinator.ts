...
export class Coordinator {
  ...
  private captureEffects(player: Player, includeExceptions = false): Array<{ typeId: string; amplifier: number; duration: number }> { try { return player.getEffects().filter(effect => effect.isValid && effect.duration > 0 && (includeExceptions || !this.isEffectSyncException(effect.typeId))).map(effect => ({ typeId: effect.typeId, amplifier: effect.amplifier, duration: effect.duration })).sort((a, b) => a.typeId.localeCompare(b.typeId)); } catch { return []; } }
  private advanceEffects(): void { if (!this.config.shareEffects || this.state.effects.length === 0) return; const effects = this.state.effects.filter(effect => !this.isEffectSyncException(effect.typeId)).map(effect => ({ ...effect, duration: effect.duration - this.config.reconciliationIntervalTicks })).filter(effect => effect.duration > 0); if (effects.length !== this.state.effects.length) this.state.dirty = true; this.state.effects = effects; }
  ...
  private applyEffects(player: Player): void { const target = new Map(this.captureEffects(player, true).map(effect => [effect.typeId, effect])); const shared = new Map(this.state.effects.map(effect => [effect.typeId, effect])); for (const typeId of target.keys()) if (!shared.has(typeId) && !this.isEffectSyncException(typeId)) player.removeEffect(typeId); for (const effect of this.state.effects) { if (this.isEffectSyncException(effect.typeId)) continue; const current = target.get(effect.typeId); if (!current || current.amplifier !== effect.amplifier || current.duration + this.config.applyLockTicks < effect.duration) player.addEffect(effect.typeId, effect.duration, { amplifier: effect.amplifier }); } }
  private isEffectSyncException(typeId: string): boolean { return this.config.effectSyncExceptions.includes(typeId); }
